...
class NLPProcessor:
    ...
    def process_query(self, query):
        ...
        # --- Nhận diện Ý định TẢI LOG CỤC BỘ (Nếu có, giữ lại quy tắc này) ---
        ...
        match_type_location = re.search(self.list_search_verbs + r'\s+(hóa\s+chất|vật\s+tư|chất)\s+(trong|từ|ở)\s+(tủ|kệ)\s+([a-zA-Z0-9\s.-]+)', query_lower)
        ...
        match_loc_status = re.search(self.list_search_verbs + r'\s+(.+)\s+(trong|từ)\s+(tủ|kệ)\s+([a-zA-Z0-9\s.-]+)', query_lower)
        if match_loc_status:
        ...
        match_type_status = re.search(self.list_search_verbs + r'\s+(hóa\s+chất|vật\s+tư|chất)(?:\s+(.+))?', query_lower)
